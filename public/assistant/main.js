...
chat = createChatView({
  state: state.chat, elements: el, agentClient, sessionService, command, uiDialogs, renderMarkdown,
...
sessions = createSessionsView({
  ...
  getRunning: chat.isRunning, loadBootstrap, showNotice, showError,
});
...
const agentEvents = createAgentEventStream({
  ...
  onStatus: (status, detail) => {
    if (status === "open" && detail.reconnected && !chat.isStreaming()) void chat.syncMessagesFromAgent();
...
function bindEvents() {
  ...
  el.stopButton.addEventListener("click", () => agentClient.stop().catch(showError));
  ...
  el.sessionSearch.addEventListener("input", () => sessions.renderSessions(state.sessions.sessions));
  el.compactButton.addEventListener("click", chat.compactSession);
  el.statsButton.addEventListener("click", chat.showStats);
...
async function loadBootstrap() {
  chat.setRuntime("正在启动 Windows Pi");
  try {
    ...
    chat.setRuntime("Windows Pi 已就绪", "ready");
  } catch (error) { chat.setRuntime("Windows Pi 启动失败", "error"); showError(error); }
}
...
function handleAgentEvent(event) {
  ...
  switch (event.type) {
    case "connected":
      ...
      if (event.state === "idle") chat.setRuntime("Windows Pi 已休眠，发送消息自动唤醒");
      else if (event.state === "error") chat.setRuntime("Windows Pi 异常退出", "error");
      else if (event.state === "busy") chat.setRuntime("宝蛋正在处理", "ready");
      else if (event.running) chat.setRuntime("Windows Pi 已连接", "ready");
      else chat.setRuntime("Windows Pi 待启动");
      break;
    case "runtime_ready": chat.setRuntime("Windows Pi 已就绪", "ready"); updateStateFromAgent(event.state || {}); break;
    case "runtime_stopping": if (event.reason === "idle") chat.setRuntime("Windows Pi 正在休眠"); break;
    case "runtime_idle": chat.setRuntime("Windows Pi 已休眠，发送消息自动唤醒"); chat.setRuntimeState(agentState); break;
    case "runtime_stopped": if (event.reason === "restart") chat.setRuntime("Windows Pi 正在重启"); break;
    case "runtime_exit": chat.setRuntime("Windows Pi 异常退出", "error"); chat.setRuntimeState(agentState); break;
    case "agent_start":
      chat.setRuntimeState(agentState); workspace.setTurnFiles({ involved: [], modified: [] });
      chat.setRuntime("宝蛋正在处理", "ready"); break;
    ...
    case "queue_update": showNotice(`待处理消息：${(event.steering?.length || 0) + (event.followUp?.length || 0)}`); break;
    ...
    case "agent_settled":
      chat.settle(agentState); chat.setRuntime("Windows Pi 已就绪", "ready");
