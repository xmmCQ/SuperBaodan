...
export class BackendManager extends EventEmitter {
  constructor(config, dependencies = {}) {
    ...
    this.baseUrl = config.baseUrl || `http://127.0.0.1:${config.port}`;
    this.spawnProcess = dependencies.spawnProcess || spawn;
    this.portAvailable = dependencies.portAvailable || isPortAvailable;
  ...
  async initialize() {
    ...
    catch { await copyFile(path.join(this.root, "desktop", "default-work-todo.md"), todoFile); }
  }
  ...
  async startRun(run) {
    try {
      ...
      if (!await this.portAvailable(this.port, "127.0.0.1")) throw new BackendStartError("PORT_IN_USE", `端口 ${this.port} 已被其他程序占用`);
      this.assertStartActive(run);
      const child = this.spawnProcess(this.nodePath, [path.join(this.root, "server.mjs")], {
        cwd: this.root,
        windowsHide: true,
  ...
  handleMessage(run, message) {
    ...
    if (message.type === "ready") {
      run.resolveReady?.({ ok: true, desktopInstanceId: run.id, port: this.port });
  ...
  async stopInternal(timeoutMs) {
    ...
    if (this.startPromise) await this.startPromise.catch((error) => { if (!isStartCancelled(error)) this.emit("start-error", error); });
    if (this.currentRun !== run || run.exitInfo) return true;
    if (!run.child) {
      this.currentRun = null;
      this.state = "idle";
      return true;
...
export function isPortAvailable(port, host = "127.0.0.1") {
  return new Promise((resolve) => {
    ...
    tester.listen({ port, host, exclusive: true }, () => tester.close(() => resolve(true)));
  });
}
