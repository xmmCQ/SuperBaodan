...
export function resolveDesktopConfig({ app, root, env = process.env }) {
  ...
  const port = parsePort(env.SUPER_BAODAN_DESKTOP_PORT, packaged ? 3212 : 3213);
...
export function createBackendEnvironment(config, runId, source = process.env) {
  ...
  Object.assign(env, {
    SUPER_BAODAN_PORT: String(config.port),
...
function parsePort(value, fallback) {
  ...
  if (!Number.isInteger(port) || port < 1024 || port > 65535) throw new Error("桌面端口必须是 1024 至 65535 之间的整数");
  return port;
}
