...
export class PromptReceipts {
  ...
  submit(id, payload, operation) {
    if (typeof id !== "string" || !/^[a-zA-Z0-9_-]{16,100}$/.test(id)) throw fault(400, "消息请求标识无效");
    this.prune();
    const hash = createHash("sha256").update(JSON.stringify(payload)).digest("hex");
    const previous = this.items.get(id);
    if (previous) {
      if (previous.hash !== hash) throw fault(409, "消息请求标识已用于其他内容");
      return previous.promise;
    }
    if (this.items.size >= this.limit) throw fault(429, "消息受理记录已满，请稍后再试");
