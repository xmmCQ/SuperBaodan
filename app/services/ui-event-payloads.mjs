...
export class UiEventPayloads {
  ...
  get(token, runtime, epoch, workspaceId) {
    ...
    if (!item || item.runtime !== runtime || item.bridge !== runtime.uiBridge || item.epoch !== epoch || item.workspaceId !== workspaceId) throw mutationError(410, '扩展界面内容已失效，请重试');
    const event = item.event || runtime.pendingUiRequests().find(request => request.id === item.id);
    if (!event) throw mutationError(410, '扩展交互已结束');
    item.fetched = true;
    return event;
  }
  remove(token) {
