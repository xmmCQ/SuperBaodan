...
export class WorkspaceService {
  ...
  async resolveExisting(relativePath = "", expected = null) {
    ...
    const resolved = await realpath(candidate).catch((error) => {
      if (error.code === "ENOENT") throw fault(404, "文件或目录不存在");
      throw error;
    });
    const rootReal = this.rootReal || await realpath(this.root);
    assertInside(resolved, rootReal);
    const info = await stat(resolved);
    if (expected === "file" && !info.isFile()) throw fault(400, "请求的路径不是文件");
    if (expected === "directory" && !info.isDirectory()) throw fault(400, "请求的路径不是目录");
  ...
  async upload(relativeDirectory = "", name, content, { overwrite = false } = {}) {
    ...
    if (existing?.isSymbolicLink()) throw fault(403, "禁止覆盖符号链接");
    if (existing?.isDirectory()) throw fault(409, "同名目录已存在");
    if (existing && !overwrite) throw fault(409, "同名文件已存在");
  ...
  async preview(relativePath) {
    ...
    if (!type) throw fault(415, "此文件类型不允许预览，或文件超过大小限制");
    if (["text", "code", "markdown"].includes(type.kind)) {
      const buffer = await readFile(file.absolute);
      if (buffer.length > MAX_TEXT_PREVIEW_BYTES) throw fault(413, "文件在读取期间超过文本预览大小限制");
      const content = buffer.toString("utf8");
      if (content.includes("\0")) throw fault(415, "检测到二进制内容，不能作为文本预览");
      return { path: file.relative, size: buffer.length, ...type, content };
    }
    return { path: file.relative, size: file.stat.size, ...type, contentUrl: `app://workbench/content?path=${encodeURIComponent(file.relative)}` };
  }

  async content(relativePath) {
    const file = await this.resolveExisting(relativePath, "file");
    const type = previewType(file.absolute, file.stat.size);
    if (!type || !["image", "pdf"].includes(type.kind)) throw fault(415, "此接口只提供允许的图片和 PDF 预览");
    return { ...file, ...type };
  }
}
...
export function validateUploadFiles(files) {
  if (!Array.isArray(files) || !files.length) throw fault(400, "请选择要上传的文件");
  if (files.length > MAX_UPLOAD_FILES) throw fault(413, `一次最多上传${MAX_UPLOAD_FILES}个文件`);
  const seen = new Set();
  let total = 0;
  const normalized = files.map((file) => {
    const name = String(file?.name || "").trim();
    const size = Number(file?.size);
    if (!name || name.length > 240 || name === "." || name === ".." || /[<>:"/\\|?*\u0000-\u001f]/.test(name) || /[. ]$/.test(name)) throw fault(400, `文件名无效：${name || "空文件名"}`);
    if (/^(con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\.|$)/i.test(name)) throw fault(400, `Windows保留文件名不可用：${name}`);
    if (!Number.isSafeInteger(size) || size < 0) throw fault(400, `文件大小无效：${name}`);
    if (size > MAX_UPLOAD_FILE_BYTES) throw fault(413, `单个文件不能超过${MAX_UPLOAD_FILE_BYTES / 1024 / 1024}MB：${name}`);
    const key = name.toLocaleLowerCase();
    if (seen.has(key)) throw fault(400, `选择的文件名称重复：${name}`);
    seen.add(key);
    total += size;
    return { name, size };
  });
  if (total > MAX_UPLOAD_TOTAL_BYTES) throw fault(413, `一次上传总大小不能超过${MAX_UPLOAD_TOTAL_BYTES / 1024 / 1024}MB`);
  return normalized;
}

export function resolveLexically(root, relativePath = "") {
  if (typeof relativePath !== "string" || relativePath.includes("\0")) throw fault(400, "工作区路径无效");
  if (path.isAbsolute(relativePath) || path.win32.isAbsolute(relativePath)) throw fault(403, "仅允许工作区相对路径");
  const segments = relativePath.split(/[\\/]+/).filter((segment) => segment && segment !== ".");
  if (segments.some((segment) => segment === "..")) throw fault(403, "禁止路径穿越");
...
function assertInside(candidate, root) {
  ...
  if (relative.startsWith("..") || path.isAbsolute(relative)) throw fault(403, "禁止访问工作区之外的路径");
}
