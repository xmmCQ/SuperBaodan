...
export class DailyRecordManager {
  ...
  async search(rawQuery) {
    ...
    if (!query) throw fault(400, "请输入搜索关键词");
    if (query.length > MAX_QUERY_LENGTH) throw fault(400, `搜索关键词不能超过${MAX_QUERY_LENGTH}个字符`);
  ...
  async update(id, input) {
    ...
    return this.mutate(async (store) => {
      ...
      if (index < 0) throw fault(404, "每日记录不存在");
...
function validateEditableInput(input) {
  ...
  if (!title) throw fault(400, "记录标题不能为空");
  if (title.length > MAX_TITLE_LENGTH) throw fault(400, `记录标题不能超过${MAX_TITLE_LENGTH}个字符`);
  if (!TYPES.has(type)) throw fault(400, "记录类型无效");
  if (time && !/^([01]\d|2[0-3]):[0-5]\d$/.test(time)) throw fault(400, "记录时间格式无效");
  if (!content.trim()) throw fault(400, "记录正文不能为空");
  if (content.length > MAX_CONTENT_LENGTH) throw fault(400, `记录正文不能超过${MAX_CONTENT_LENGTH}个字符`);
...
function validateId(value) {
  if (typeof value !== "string" || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value)) {
    throw fault(400, "每日记录ID无效");
  }
  return value;
}

function validateDate(value) {
  if (typeof value !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(value)) throw fault(400, "记录日期格式无效");
  const [year, month, day] = value.split("-").map(Number);
  const date = new Date(Date.UTC(year, month - 1, day));
  if (date.getUTCFullYear() !== year || date.getUTCMonth() !== month - 1 || date.getUTCDate() !== day) throw fault(400, "记录日期无效");
  return value;
}

function validateMonth(value) {
  if (typeof value !== "string" || !/^\d{4}-(0[1-9]|1[0-2])$/.test(value)) throw fault(400, "月份格式无效");
  return value;
}
...
function assertRevision(revision, current) {
  if (!revision) throw fault(400, "缺少记录版本，请刷新后重试");
  if (revision !== current) throw fault(409, "记录已在其他位置更新，请刷新后重试");
}
