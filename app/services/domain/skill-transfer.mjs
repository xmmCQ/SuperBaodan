...
export async function skillTreeDigest(root, { maxBytes = Infinity } = {}) {
  ...
  async function walk(directory) {
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink()) throw fault(403, '不能转换包含链接或异常目录的 Skill');
    for (const name of (await readdir(directory)).sort()) {
      if (++count > 10000) throw fault(413, 'Skill 文件过多，请手动迁移');
      const file = path.join(directory, name), stat = await lstat(file), relative = path.relative(root, file);
      if (stat.isSymbolicLink()) throw fault(403, '不能转换包含符号链接的 Skill');
      if (stat.isDirectory()) { entries.push(['dir', relative]); await walk(file); }
      else if (stat.isFile()) {
        bytes += stat.size;
        if (bytes > maxBytes) throw fault(413, 'Skill 文件过大，未完成内容核验');
        const hash = createHash('sha256');
        for await (const chunk of createReadStream(file)) hash.update(chunk);
        entries.push(['file', relative, stat.size, hash.digest('hex')]);
      } else throw fault(403, '不能转换包含异常文件的 Skill');
    }
  }
  await walk(root);
...
async function safeRoot(root) {
  ...
  if (path.resolve(await realpath(existing)).toLowerCase() !== path.resolve(existing).toLowerCase()) throw fault(403, 'Skill 目标目录不能经过符号链接');
  await mkdir(root, { recursive: true });
}
export async function transferSkillDirectory({ source, target, backupDir, expectedRevision, removeSource = rm }) {
  if (inside(target, source) || inside(source, target) || inside(backupDir, source) || inside(backupDir, target)) throw fault(403, 'Skill 目录不能互相嵌套');
  await safeRoot(path.dirname(target));
  try { await lstat(target); throw fault(409, '目标位置已有同名 Skill，不会覆盖'); }
  catch (error) { if (error.code !== 'ENOENT') throw error; }
  const digest = await skillTreeDigest(source);
  if (expectedRevision && JSON.parse(digest).find(entry => entry[0] === 'file' && entry[1] === 'SKILL.md')?.[3] !== expectedRevision) throw fault(409, 'Skill 已变化，请刷新后重试');
  ...
  if (await skillTreeDigest(saved) !== digest) throw fault(409, '备份期间 Skill 已变化，未转换');
  ...
  try {
    ...
    if (await skillTreeDigest(target) !== digest || await skillTreeDigest(source) !== digest) throw fault(409, '转换期间 Skill 已变化，已取消');
    deleting = true;
    ...
    if (deleting) {
      try {
        ...
        throw fault(500, `转换失败，原目录恢复未完成；目标副本与备份已保留。备份：${backup}；${restoreError.message}`);
      }
    }
    if (reserved) await rm(target, { recursive: true, force: true }).catch(() => { error.message += `；目标残留请手动清理，备份：${backup}`; });
    if (error.code === 'EEXIST') throw fault(409, '目标位置已有同名 Skill，不会覆盖');
    throw error;
  }
}
