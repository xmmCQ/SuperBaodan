import { SkillCli, buildSkillCliArgs, validateSkillPackage, normalizeScope, skillFolder } from './skill-cli.mjs';
import { SKILL_NAME_RE, DISABLE_KEY, scalarFromSpan, validateSkillName, parseSkillMarkdown, buildSkillMarkdown, updateStructuredSkillMarkdown, setDisableModelInvocation } from './skill-markdown.mjs';
...
export class SkillManager {
  constructor({ agentDir, cwd, backupDir, piAdmin, env = process.env, fetchImpl = fetch, execFileImpl, loadSdk = loadPiSdk, openDirectoryImpl = openSkillDirectory, log = console }) {
    ...
    this.cli = new SkillCli({ env, execFileImpl, log });
  ...
  async list() {
    ...
    if (typeof DefaultResourceLoader !== "function") throw fault(503, "Windows Pi SDK不支持Skill管理，请更新Pi");
    ...
    const cliAvailable = await this.cli.isCliAvailable();
  ...
  async search(query) {
    ...
    catch (error) { throw fault(502, `skills.sh连接失败：${error.message}`); }
    if (!response.ok) throw fault(502, `skills.sh搜索失败：HTTP ${response.status}`);
  ...
  install(payload) {
    ...
    return this.mutateWithMaintenance(async () => {
      ...
      try {
        await this.cli.runCli(buildSkillCliArgs("install", { package: pkg, scope }), { cwd: workspaceLayout(this.cwd).parkRoot, sensitivePaths: [this.homeDir, this.agentDir, this.cwd] });
  ...
  update(payload) {
    ...
    return this.mutateWithMaintenance(async () => {
      ...
      try {
        await this.cli.runCli(buildSkillCliArgs("update", { scope, name: skill.name, install: skill.install }), { cwd: workspaceLayout(this.cwd).parkRoot, sensitivePaths: [this.homeDir, this.agentDir, this.cwd] });
        return { updated: true };
      } catch (error) {
        await this.rollback(snapshot).catch((rollbackError) => { error.message += `；自动恢复失败，备份位于 ${snapshot.dir}：${rollbackError.message}`; });
        throw error;
  ...
  uninstall(payload) {
    ...
    return this.mutateWithMaintenance(async () => {
      await this.cli.requireCli();
      await this.findManagedSkill(pkg, scope, name);
      const snapshot = await this.backupMutation(`uninstall-${scope}-${name}`, { scope, name });
      try {
        await this.cli.runCli(buildSkillCliArgs("uninstall", { scope, name }), { cwd: workspaceLayout(this.cwd).parkRoot, sensitivePaths: [this.homeDir, this.agentDir, this.cwd] });
  ...
  setInvocation(payload) {
    const scope = normalizeScope(payload.scope);
    const name = validateSkillName(payload.name);
    if (typeof payload.disableModelInvocation !== "boolean") throw fault(400, "缺少自动调用设置");
    return this.mutateWithMaintenance(async () => {
      const skill = await this.findSkill(scope, name);
      if (scope === "other") throw fault(403, "该Skill为只读来源");
  ...
  createCustom(payload) {
    ...
    return this.mutateWithMaintenance(async () => {
      ...
      if (existsSync(directory)) throw fault(409, `Skill已存在：${name}`);
  ...
  updateCustom(payload) {
    ...
    return this.mutateWithMaintenance(async () => {
      ...
      const locks = readSkillLocks({ homeDir: this.homeDir, cwd: workspaceLayout(this.cwd).parkRoot });
      if (annotateInstall({ name }, locks, scope)) throw fault(409, "skills.sh管理的Skill不能直接编辑，请使用更新功能");
  ...
  deleteCustom(payload) {
    ...
    return this.mutateWithMaintenance(async () => {
      ...
      await this.assertSafeExistingSkill(scope, name, filePath);
      const locks = readSkillLocks({ homeDir: this.homeDir, cwd: workspaceLayout(this.cwd).parkRoot });
      if (annotateInstall({ name }, locks, scope)) throw fault(409, "skills.sh管理的Skill必须使用卸载功能");
  ...
  async openDirectory(payload) {
    if (typeof payload.id !== 'string' || !payload.id) throw fault(400, '缺少 Skill 标识');
  ...
  async transfer(payload) {
    ...
    try { return await this.mutateWithMaintenance(async () => {
      const skill = await this.findSkill(scope, name);
      if (!skill.writable || skill.install) throw fault(403, '只读或软件包管理的 Skill 不支持互转');
      if (!payload.revision || payload.revision !== skill.revision || payload.id !== skill.id) throw fault(409, 'Skill 已变化，请刷新后重试');
      const locks = readSkillLocks({ homeDir: this.homeDir, cwd: workspaceLayout(this.cwd).parkRoot });
      if (annotateInstall({ name }, locks, targetScope)) throw fault(409, '目标范围已有同名安装记录，不会覆盖');
  ...
  async validateRawContent(content, expectedName) {
    ...
    if (parsed.name !== expectedName) throw fault(400, "frontmatter中的name必须与Skill目录名称一致");
    const sdk = await this.loadSdk(this.env);
    if (typeof sdk.parseFrontmatter === "function") {
      ...
      } catch (error) { throw fault(400, `YAML frontmatter无效：${error.message}`); }
    }
  }
  ...
  async findSkill(scope, name) {
    ...
    if (!skill) throw fault(404, "未找到该Skill，请刷新后重试");
    return skill;
  }
  ...
  async findManagedSkill(pkg, scope, expectedName) {
    ...
    if (!skill) throw fault(404, "未找到对应的skills.sh安装记录");
    return skill;
  }
  ...
  async assertSafeRoot(root) {
    ...
    if (path.resolve(resolved).toLowerCase() !== path.resolve(root).toLowerCase()) throw fault(403, "Skill根目录不能是符号链接");
  }
  ...
  async assertSafeManagedSkill(skill) {
    ...
    const expected = path.join(this.rootForScope(scope), name, "SKILL.md");
    if (path.resolve(skill.filePath).toLowerCase() !== path.resolve(expected).toLowerCase()) throw fault(403, "skills.sh Skill路径与安装记录不一致");
    const realFile = await realpath(expected).catch(() => { throw fault(404, "Skill文件不存在"); });
    ...
    if (!allowedRoots.filter(Boolean).some((root) => isInside(realFile, root))) throw fault(403, "skills.sh Skill真实路径越界");
    const fileStat = await lstat(realFile);
    if (!fileStat.isFile() || fileStat.isSymbolicLink()) throw fault(403, "拒绝操作异常Skill文件");
  }

  async assertSafeExistingSkill(scope, name, filePath) {
    const root = this.rootForScope(scope);
    const expected = path.join(root, validateSkillName(name), "SKILL.md");
    if (path.resolve(expected).toLowerCase() !== path.resolve(filePath).toLowerCase()) throw fault(403, "Skill路径越界");
    await this.assertSafeRoot(root);
    const directory = path.dirname(expected);
    const [dirStat, fileStat] = await Promise.all([lstat(directory), lstat(expected)]).catch((error) => {
      if (error.code === "ENOENT") throw fault(404, "Skill文件不存在");
      throw error;
    });
    if (dirStat.isSymbolicLink() || fileStat.isSymbolicLink() || !dirStat.isDirectory() || !fileStat.isFile()) throw fault(403, "拒绝操作符号链接或异常Skill路径");
    const [realRoot, realFile] = await Promise.all([realpath(root), realpath(expected)]);
    if (!isInside(realFile, realRoot)) throw fault(403, "Skill真实路径越界");
  }
