...
import { emptyDocuments, validateDocuments, normalizeTarget, MAX_BYTES, fail } from '../../shared/work-documents.js';
const hash = text => createHash('sha256').update(text).digest('hex');
export class WorkDocuments {
  constructor({ filePath, launch = null, recycle = recycleWorkDocument }) { this.filePath = filePath; this.launch = launch; this.recycle = recycle; this.queue = Promise.resolve(); this.opening = new Set(); }
  async read() {
    let raw;
  ...
  async open(body) {
    ...
    try {
      return await this.enqueue(async () => {
        ...
        if (this.launch) { await this.launch(real); return { ok: true, opened: true, message: '已交给系统打开' }; }
        return { path: real };
      });
