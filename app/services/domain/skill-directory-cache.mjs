...
export class SkillDirectoryCache {
  ...
  async resolve(id) {
    ...
    if (!entry) throw fault(404, 'Skill 已变化，请刷新技能列表后重试');
    try {
      const [directory, file] = await Promise.all([realpath(path.dirname(entry.filePath)), realpath(entry.filePath)]);
      if (!samePath(directory, entry.directory) || !samePath(file, entry.file)) throw fault(409, 'Skill 路径已变化，请刷新技能列表后重试');
      const [dirStat, fileStat] = await Promise.all([lstat(directory), lstat(file)]);
      if (!dirStat.isDirectory() || !fileStat.isFile()) throw fault(409, 'Skill 路径无效，请刷新技能列表后重试');
      if (this.entries.get(id) !== entry) throw fault(409, 'Skill 列表已变化，请重试');
      return directory;
    } catch (error) {
      if (this.entries.get(id) === entry) this.entries.delete(id);
      if (error.statusCode) throw error;
      throw fault(404, 'Skill 目录或文件不存在，请刷新技能列表后重试');
    }
  }
}
