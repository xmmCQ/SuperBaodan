export function createWorkspaceSwitcher({ trigger, invoke, uiDialogs, showPathTooltip = true, hasDraft = () => false, getDraftWarning = () => '切换工作区将清空当前未发送的内容。', clearDraft = () => {}, onActivating = () => {}, onActivated = async () => {}, onError = console.error }) {
  ...
  async function load() {
    data = await invoke("workspaces.list", {});
    updateTrigger();
    render();
    return data;
  }
  ...
  async function activate(item) {
    ...
    try {
      const result = await invoke("workspaces.activate", { id: item.id });
      finishTransition?.();
  ...
  async function renameWorkspace(item) {
    ...
    try {
      await invoke("workspaces.rename", { id: item.id, name: name.trim() });
      await load();
    } catch (error) { onError(error); }
  }
  ...
  async function removeWorkspace(item) {
    ...
    if (!confirmed) return;
    try {
      await invoke("workspaces.remove", { id: item.id });
      await load();
    } catch (error) { onError(error); }
  }
  ...
  async function browse(candidate) {
    ...
    const result = await invoke("workspaces.browse", { path: candidate });
  ...
  async function addWorkspace(event) {
    ...
    try {
      const created = await invoke("workspaces.add", { name, path: selectedPath });
      picker.dialog.close();
      await load();
