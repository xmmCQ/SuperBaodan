...
// A deliberate conversation reset supersedes layout anchors from the old DOM.
export function cancelReadingAdjustment(container) {
  const record = adjustments.get(container);
  if (!record) return;
  container.style.scrollBehavior = record.behavior;
  container.style.overflowAnchor = record.anchorStyle;
  delete container.dataset.readingAdjustment;
  adjustments.delete(container);
}
...
export function preserveReadingPositions(containers, change) {
  const records = [...new Set(containers.filter((node) => node?.clientHeight))].map((node) => {
    const previous = adjustments.get(node);
    const record = { node, position: previous?.position || captureReadingPosition(node), behavior: previous?.behavior ?? node.style.scrollBehavior, anchorStyle: previous?.anchorStyle ?? node.style.overflowAnchor };
