...
async function loadSkills({ preserveSelection = false } = {}) {
  ...
  try {
    const data = await invoke("skills.list", {});
    if (!current()) return;
...
async function searchSkillMarket(query) {
  ...
  try {
    ...
    const data = await invoke("skills.search", { q: state.skillMarketQuery });
...
async function installSkill(item, scope) {
  ...
  await runSkillMutation(() => invoke("skills.install", { package: item.package, scope }), "安装成功");
}
...
async function updateSkill(skill) {
  ...
  await runSkillMutation(() => invoke("skills.update", { package: skill.install.package, scope: skill.scope }), "更新成功");
}

async function uninstallSkill(skill) {
  if (!await uiDialogs.confirm(`来源：${skill.install.source}\n范围：${scopeLabel(skill.scope)}\n卸载前会自动备份。`, { title: `确认卸载 ${skill.name}？`, danger: true, confirmText: "确认卸载" })) return;
  await runSkillMutation(() => invoke("skills.uninstall", { name: skill.name, package: skill.install.package, scope: skill.scope }), "卸载成功");
}

async function toggleSkillInvocation(skill, checkbox) {
  checkbox.disabled = true;
  try {
    const result = await invoke("skills.setInvocation", { name: skill.name, scope: skill.scope, disableModelInvocation: !checkbox.checked });
...
async function saveCustomSkill(skill) {
  ...
  await runSkillMutation(async () => {
    const result = await invoke("skills.updateCustom", { ...(payload) });
...
async function createCustomSkill() {
  ...
  await runSkillMutation(() => invoke("skills.createCustom", { ...(payload) }), "创建成功");
}
...
async function transferSkill(skill) {
  ...
  await runSkillMutation(() => invoke("skills.transfer", { id: skill.id, name: skill.name, scope: skill.scope, revision: skill.revision, workspaceId: listedWorkspaceId }), skill.scope === 'global' ? '已移入当前项目' : '已提升为全局');
}
...
async function checkSkillUpdates(skill = null) {
  ...
  try {
    ...
    const data = await invoke("skills.checkUpdates", { ...(body) });
...
async function openSkillFolder(skill) {
  if (!listedWorkspaceId || listedWorkspaceId !== getWorkspaceId()) return showSettingsToast('工作区已变化，请刷新技能列表后重试', 'error');
  try { await invoke("skills.openDirectory", { id: skill.id, workspaceId: listedWorkspaceId }); }
  catch (error) { showSettingsToast(error.message, 'error'); }
}
