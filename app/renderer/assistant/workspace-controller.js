...
async function uploadWorkspaceFiles() {
  ...
  try {
    const check = await invoke("files.checkUpload", { ...(workspacePayload({ directory: "", files: files.map((file) => ({ name: file.name, size: file.size })) })) });
    ...
    for (let index = 0; index < selected.length; index += 1) {
      ...
      try {
        const data = await invoke("files.upload", { name: file.name, overwrite: overwrite, workspaceId: state.workspace?.id, content: new Uint8Array(await file.arrayBuffer()) });
...
function scheduleWorkspaceReload(message = "") {
  ...
  return { setWorkspace, workspace, setTurnFiles, turnFiles, toggleWorkspace, setWorkspaceOpen, refreshWorkspaceTreeIfOpen, loadWorkspaceTree, uploadWorkspaceFiles, searchWorkspaceFiles, previewWorkspaceFile, handlePromptInput, insertFileReference, scheduleWorkspaceReload };
}
