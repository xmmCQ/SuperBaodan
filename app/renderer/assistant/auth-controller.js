...
function openLoginPage(url) {
  const openExternal = window.workbench?.openExternal;
  if (!openExternal) return false;
  Promise.resolve(openExternal(url)).catch(showError);
  return true;
}

async function loadAccounts() {
  const data = await invoke("auth.providers", {});
  providerCatalog = {
...
async function logoutProvider(providerId, type) {
  ...
  try {
    await invoke((type === "oauth" ? "auth.logout" : "auth.deleteKey"), { id: providerId });
    showNotice("已退出登录");
...
function startOAuthLogin(provider) {
  ...
  let closed = false;
  const source = {
    close() { if (closed) return; closed = true; unsubscribe(); void invoke('auth.cancel', { subscriptionId }).catch(showError); },
  };
  const unsubscribe = window.workbench.onEvent(({ topic, event }) => {
    if (!closed && topic === 'auth' && event.subscriptionId === subscriptionId) source.onmessage?.({ data: JSON.stringify(event) });
  });
  void invoke('auth.start', { providerId: provider.id, subscriptionId }).catch(error => { source.close(); showError(error); });
  ...
  source.onmessage = (message) => {
    ...
    if (event.type === "auth") renderOAuthAction(provider, event);
      ...
      source.close(); state.activeLoginSource = null;
...
function renderOAuthAction(provider, event) {
  ...
  const opened = openLoginPage(event.url);
...
function renderDeviceCode(provider, event) {
  el.loginProgress.replaceChildren();
  const opened = openLoginPage(event.verificationUri);
...
function renderLoginInputControls(providerId, token, placeholder, options) {
  if (Array.isArray(options) && options.length) {
    ...
    for (const option of options) {
      ...
      button.addEventListener("click", async () => {
        await submitLoginInput(providerId, token, option.id);
      });
      wrap.append(button);
    }
  ...
  button.addEventListener("click", async () => {
    await submitLoginInput(providerId, token, input.value);
  });
...
async function submitLoginInput(providerId, token, value) {
  try {
    await invoke("auth.input", { id: providerId, token, value });
...
async function submitSecretInput() {
  ...
  try {
    await invoke("auth.saveKey", { id: provider.id, apiKey: el.secretInput.value });
  ...
  function closeActiveLogin() { state.activeLoginSource?.close(); state.activeLoginSource = null; }
