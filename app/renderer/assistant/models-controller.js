...
async function resolveEnabledModels(bootstrapEnabledModels) {
  ...
  try {
    const catalog = await invoke("models.catalog", {});
...
async function loadModelsConfig() {
  state.modelsConfig = await invoke("models.read", {});
...
async function saveModelsConfig() {
  try {
    ...
    await invoke("models.save", { ...(state.modelsConfig) });
...
async function testConfiguredModel() {
  try { commitProviderForm(); const provider = state.modelsConfig.providers[state.providerKey]; const model = provider.models?.[state.modelIndex]; if (!model) throw new Error("请先选择模型"); el.testModelResult.textContent = "正在测试……"; const result = await invoke("models.test", { providerName: state.providerKey, provider, model }); el.testModelResult.textContent = `成功 · ${result.latencyMs}ms · ${result.responseText || "OK"}`; }
