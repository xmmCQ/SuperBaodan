...
async function loadAgentStatus() {
  try {
    const data = await invoke("system.status", {});
...
function renderChatMessages(messages, { hideTrailingAssistant = false, forceScroll = true } = {}) {
  ...
  if (forceScroll) cancelReadingAdjustment(el.chatMessages);
...
function resetStaleLive() { if (liveCurrent?.() === false) { state.liveAssistant?.remove(); state.liveAssistant = null; liveCurrent = null; } }
...
function scrollChat(force = false) {
  if (force) { cancelReadingAdjustment(el.chatMessages); stickToChatBottom = true; }
