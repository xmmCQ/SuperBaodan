import { stripTaskKind } from '../../shared/task-fields.js';

export const CARD_ORDER_PREFIX = 'super-baodan.day-order.v1.';

export function taskOrderKey(task) {
  const text = stripTaskKind(String(task.editableText ?? task.text ?? '')).replace(/(?:⏳|📅|✅)\s*\d{4}-\d{2}-\d{2}/gu, '').replace(/🔁\s*every\s+(?:day|week|month|year)/gi, '').replace(/\s+/g, ' ').trim();
