...
export function createWorkApps({ trigger, invoke, uiDialogs, toast, escapeHtml: esc, openAll }) {
  ...
  async function open() {
    ...
    busy = true;
    try {
      const data = await invoke("apps.read", {}); apps = data.apps; revision = data.revision; original = JSON.stringify(apps);
  ...
  list.addEventListener('click', async event => {
    ...
    if (button.hasAttribute('data-run')) {
      ...
      try { const data = await invoke("apps.open", { ...({ id: apps[index].id, revision }) }); notice.textContent = [...data.results.map(r => `${r.name}：${r.message}`), data.warning].filter(Boolean).join('；'); }
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      const data = await invoke("apps.save", { ...({ revision, apps }) });
