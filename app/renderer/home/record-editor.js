...
export function createRecordEditor({ state, elements: el, invoke, uiDialogs, renderMarkdown, toast, onSaved, onConflict }) {
  ...
  async function submit(event) {
    ...
    try {
      const data = await invoke((current ? "records.update" : "records.create"), { id: current?.id, ...(body) });
      hide();
