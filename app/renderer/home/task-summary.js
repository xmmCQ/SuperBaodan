export function createTaskSummary(el) {
  ...
  let current = null, suspended = false, savedScroll = 0, savedFocus = null;
  function title() {
  ...
  function open(kind, restore = false) {
    current = kind;
    el.newLongtermTaskButton.classList.toggle('hidden', kind !== 'longterm');
    ...
    const focus = restore && savedFocus?.isConnected ? savedFocus : restore && kind === 'longterm' ? el.newLongtermTaskButton : el.closeTaskSummary;
  ...
  return {
    update(kind, count) {
      ...
      entry.button.classList.toggle('hidden', kind !== 'longterm' && count === 0);
    ...
    suspend() {
      ...
      savedFocus = dialog.contains(document.activeElement) ? document.activeElement : null;
      suspended = true;
      dialog.close();
    },
    resume() {
