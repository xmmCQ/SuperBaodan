import { validateDocuments, normalizeTarget, UNCATEGORIZED, targetKey } from '../../shared/work-documents.js';
import { parseDocumentMarkdown, checkImportRow, mergeDocumentImport } from '../../shared/work-document-import.js';
...
export function createWorkDocuments({ trigger, invoke, uiDialogs }) {
  ...
  async function load(throwOnError = false) {
    ...
    try {
      const next = await invoke("documents.read", {}); validateDocuments(next);
  ...
  async function saveOrder(kind, from, to, after) {
    ...
    try {
      ...
      data = await invoke("documents.save", { ...next, revision });
      say('');
    } catch (error) {
  ...
  async function commit(next, silent = false) {
    ...
    busy = true; controls();
    try { data = await invoke("documents.save", { ...validated, revision: data.revision }); say(silent ? '' : '已保存'); }
  ...
  function render() {
    ...
    for (const doc of visible) {
      ...
      link.addEventListener('click', event => {
        ...
        link.disabled = true;
        void invoke("documents.open", { id: doc.id, revision: data.revision })
    ...
    documentDialog({ title: doc ? '编辑文档入口' : '添加文档', uiDialogs, reload: () => load(true),
      build: content => {
        ...
        const browse = button('浏览本地文件', async () => {
          ...
          try {
            const result = await invoke("documents.pick", {}, { signal: controller.signal, timeout: 185000 });
        ...
        const oldLabel = target.parentElement;
        const pathField = node('div', '', 'wd-path-field'), pathLabel = node('label', '文件路径或网址'), pathRow = node('div', '', 'wd-path-row');
        target.id = `wd-path-${crypto.randomUUID()}`; pathLabel.htmlFor = target.id;
        pathRow.append(target, browse); pathField.append(pathLabel, pathRow, pickerStatus);
        oldLabel.replaceWith(pathField);
  ...
  async function deleteDocument(doc) {
    ...
    try {
      const result = await invoke("documents.remove", { id: doc.id, revision: data.revision, recycleSource: recycleEnabled });
