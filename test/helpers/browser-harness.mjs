...
export async function launchBrowser({ width, height } = {}) {
  ...
  return {
    ...
    async setViewport(width, height, deviceScaleFactor = 1) {
      await connection.command('Emulation.setDeviceMetricsOverride', { width, height, deviceScaleFactor, mobile: false });
    },
    ...
    async close() {
      ...
      try { await profile.cleanup(); }
      catch (error) {
        if (!['EBUSY', 'EPERM', 'ENOTEMPTY'].includes(error.code)) throw error;
        // Only the unique disposable profile created by this launch is eligible.
        await stopEdgeForProfile(profilePath);
        await profile.cleanup();
      }
    },
