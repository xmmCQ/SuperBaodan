...
test("模型配置支持供应商搜索、常用优先和更多供应商折叠", () => {
  ...
  assert.match(assistantCss, /\.provider-search-row input \{[\s\S]*min-height: var\(--ui-toolbar-height\)/);
  assert.match(assistantCss, /\.provider-more summary \{[\s\S]*min-height: var\(--ui-toolbar-height\)/);
});
...
test("重点小按钮满足最小热区并保留小图标", () => {
  ...
  assert.match(sessions, /icon-action--compact session-more/);
  assert.match(sessions, /'pencil', renameSession/);
  assert.match(sessions, /'trash-2', deleteSession/);
  assert.match(sessions, /setAttribute\('role', 'menu'\)/);
  assert.match(sessions, /event\.stopPropagation\(\)/);
  assert.doesNotMatch(sessions, /textContent = "(?:✎|×)"/);
  assert.match(iconCss, /width: var\(--icon-size, var\(--ui-toolbar-height\)\)/);
  assert.match(iconCss, /height: var\(--icon-size, var\(--ui-toolbar-height\)\)/);
  assert.match(iconCss, /--icon-size: var\(--ui-compact-height\)/);
  assert.match(iconCss, /\.icon-action svg \{ width: var\(--icon-glyph-size, 18px\)/);
});
