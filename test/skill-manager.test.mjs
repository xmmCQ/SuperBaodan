...
import { SkillManager, annotateInstall } from '../app/services/domain/skill-manager.mjs';
import { buildSkillMarkdown, parseSkillMarkdown, setDisableModelInvocation, updateStructuredSkillMarkdown, validateSkillName } from '../app/services/domain/skill-markdown.mjs';
import { buildSkillCliArgs, validateSkillPackage } from '../app/services/domain/skill-cli.mjs';
...
test('卸载保留安装记录校验，校验失败不备份也不执行CLI', async () => {
  const manager = Object.create(SkillManager.prototype);
  let checked = false;
  manager.mutateWithMaintenance = operation => operation();
  manager.cli = { requireCli: async () => {}, runCli: async () => assert.fail('校验失败后不得执行CLI') };
  manager.findManagedSkill = async () => { checked = true; throw new Error('无效安装记录'); };
  manager.backupMutation = async () => assert.fail('校验失败后不得继续');
  await assert.rejects(manager.uninstall({ scope: 'project', name: 'demo', package: 'owner/repo@demo' }), /无效安装记录/);
