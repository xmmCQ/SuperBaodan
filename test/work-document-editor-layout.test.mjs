...
test('文档入口编辑器隐藏空提示并收紧底部，有提示时正常展开', { timeout: 20000 }, async t => {
  ...
  const layout = await browser.evaluate(`(()=>{
    ...
    return {
      ...
      path:(()=>{const a=editor.querySelector('[aria-label="文件路径或网址"]').getBoundingClientRect(),b=editor.querySelector('.wd-browse-file').getBoundingClientRect();return {sameRow:Math.abs(a.top-b.top)<1,inputHeight:a.height,buttonHeight:b.height,separate:a.right<b.left,inside:b.right<=editor.getBoundingClientRect().right};})(),
    };
  })()`);
  ...
  assert.deepEqual(layout.path, {sameRow:true,inputHeight:40,buttonHeight:40,separate:true,inside:true});
