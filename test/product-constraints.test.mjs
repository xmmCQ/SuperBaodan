...
const assistantHtml = await readFile(path.join(ROOT, "app/renderer/assistant.html"), "utf8");
const assistantSource = await readBundle("assistant");
const serverSource = await readBundle("server");

async function publicSources(directory = path.join(ROOT, "app", "renderer")) {
  const output = [];
...
test("会话分支能力保持移除", async () => {
  const commands = await readFile(path.join(ROOT, "app/services/domain/agent-commands.mjs"), "utf8");
...
test("工作区不执行或展示Git状态", async () => {
  const workspace = await readFile(path.join(ROOT, "app/services/domain/workspace.mjs"), "utf8");
