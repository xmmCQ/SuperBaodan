...
import { launchWorkApps } from '../app/services/domain/launch-work-apps.mjs';
...
test('软件API支持配置与单独启动，拒绝错误版本', async t => {
  ...
  const context = { config: { host: '127.0.0.1', port: 0, publicDir: path.resolve('public') }, workApps: manager, attachServer() {} };
...
test('软件启动执行器保持参数、标准输入编码和输出格式，不实际启动软件', async () => {
  let input;
  const execute = (file, args, options) => {
    assert.equal(file, 'powershell.exe');
    assert.deepEqual(args, ['-NoProfile','-ExecutionPolicy','Bypass','-File','C:\\test\\launcher.ps1','-ReadAppsFromStdin']);
    assert.deepEqual(options, { timeout: 30000, maxBuffer: 1024 * 1024, encoding: 'utf8', windowsHide: true });
    const result = Promise.resolve({ stdout: '\uFEFF {"status":"ok"}\n', stderr: ' warning \n' });
    result.child = { stdin: { on(name, handler) { assert.equal(name, 'error'); handler(new Error('ignored stdin close')); }, end(value) { input = value; } } };
    return result;
  };
  assert.deepEqual(await launchWorkApps(apps, { appScript: 'C:\\test\\launcher.ps1', execFileImpl: execute }), { results: [{ status: 'ok' }], warning: 'warning' });
  assert.deepEqual(JSON.parse(Buffer.from(input, 'base64').toString('utf8')), apps);
});
