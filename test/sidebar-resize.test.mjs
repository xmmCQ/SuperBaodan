...
test('侧栏边界在常规窗口保留520px聊天，小窗口保持180px侧栏', () => {
  ...
  assert.deepEqual(sidebarBounds(950, 330), { min: 180, max: 180 });
});
...
test('左侧栏实时伸缩、保留阅读位置、记忆宽度，与右侧边界兼容', { timeout: 25000 }, async (t) => {
  ...
  assert.equal(await browser.evaluate('Math.round(sessionSidebar.getBoundingClientRect().width)'), 240);
  ...
  assert.equal(await browser.evaluate('window.liveSidebarWidth'), 365);
  ...
  assert.equal(await browser.evaluate("localStorage.getItem('super-baodan.sidebar-width.v1')"), '365');
  ...
  assert.equal(await browser.evaluate('Math.round(sessionSidebar.getBoundingClientRect().width)'), 365);
  ...
  assert.equal(await browser.evaluate('Math.round(sessionSidebar.getBoundingClientRect().width)'), 260);
