...
test('日历工作提示和轻弹窗使用隔离数据', { timeout: 40000 }, async (t) => {
  ...
  await browser.evaluate("longtermTasks.querySelector('[data-action=edit]').click(); taskRecurrence.value=''; taskEndDate.value=''; taskStartDate.value=''; taskForm.requestSubmit()");
  await browser.waitFor("taskSummaryDialog.open && !openLongtermTasks.classList.contains('hidden') && longtermCount.textContent === '1' && longtermTasks.innerText.includes('持续测试')");
  await browser.evaluate('closeTaskSummary.click()');
  assert.equal(await browser.evaluate('document.activeElement === openLongtermTasks'), true);
  ...
  for (const [scenario, overdue, longterm] of [['overdue-longterm', 1, 1], ['overdue', 1, 0], ['longterm', 0, 1], ['none', 0, 0]]) {
    await browser.waitFor('!refreshButton.disabled');
    ...
    assert.equal(await browser.evaluate(`taskSummaryHints.classList.contains('hidden')`), false);
  }
  await browser.waitFor('!refreshButton.disabled');
  ...
  assert.equal(await browser.evaluate("newLongtermTaskButton.classList.contains('hidden')"), true);
