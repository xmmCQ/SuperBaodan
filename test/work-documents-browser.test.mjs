...
async function setup(t) {
  ...
  assert.equal(await browser.evaluate("getComputedStyle(document.querySelector('.wd-dialog')).backgroundColor"), 'rgba(255, 255, 255, 0.35)');
  ...
  assert.equal(await browser.evaluate("getComputedStyle(document.querySelector('.wd-dialog h3')).fontSize"), '18px');
  assert.equal(await browser.evaluate("[...document.querySelectorAll('.wd-toolbar button,.wd-download-template,.wd-dialog .modal-heading button')].every(n=>getComputedStyle(n).fontSize==='13px' && getComputedStyle(n).borderRadius==='9999px' && n.getBoundingClientRect().height>=32)"), true);
  ...
  const click = (selector, text) => browser.evaluate(`[...document.querySelectorAll(${JSON.stringify(selector)})].find(b=>b.textContent===${JSON.stringify(text)}).click()`);
  ...
  return { fixture, browser, click, fill, save };
}
test('文档弹窗懒加载、分类/入口管理、搜索、打开防重及只删除入口', { timeout: 25000 }, async t => {
  ...
  await click('.wd-row-actions button', '删除');
...
test('Markdown预览与修复、重复导入、非链接正文不出浏览器、冲突保留草稿', { timeout: 25000 }, async t => {
  ...
  await click('.wd-row-actions button', '编辑'); await fill('文档名称', '保留的新名称');
...
test('文档分类隐藏空项，未分类置底，选中分类清空后回到全部', { timeout: 25000 }, async t => {
  if (!edgeAvailable()) return t.skip('需要 Edge');
  const { fixture, browser, click } = await setup(t);
  ...
  await click('.wd-toolbar button', '重新读取');
  await browser.waitFor("document.querySelectorAll('.wd-row').length===2");
  assert.deepEqual(await browser.evaluate("[...document.querySelectorAll('.wd-category > button:not(.wd-drag-handle)')].map(n=>n.textContent)"), ['全部 · 2','业务资料 · 1','未分类 · 1']);
  await click('.wd-category > button:not(.wd-drag-handle)', '业务资料 · 1');
  ...
  await click('.wd-toolbar button', '重新读取');
  await browser.waitFor("document.querySelector('.wd-category > button:not(.wd-drag-handle).active')?.textContent==='全部 · 1'");
  assert.deepEqual(await browser.evaluate("[...document.querySelectorAll('.wd-category > button:not(.wd-drag-handle)')].map(n=>n.textContent)"), ['全部 · 1','未分类 · 1']);
...
test('源文件删除需确认勾选，关闭重置，未勾选无确认仅删入口', { timeout: 25000 }, async t => {
  ...
  await click('.wd-toolbar button','重新读取');await browser.waitFor("document.querySelectorAll('.wd-row').length===1");
