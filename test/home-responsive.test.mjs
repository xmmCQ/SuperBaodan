...
test('首页真实桌面布局保持字号、三列与区域滚动', { timeout: 90000 }, async (t) => {
  ...
  for (const [width, height] of [[960,640], [1024,768], [1280,720], [1440,900], [1920,1080], [2048,1152], [2560,1440], [3840,2160]]) {
    await t.test(`${width}×${height}`, async () => {
      ...
      try {
        ...
        const geometry = JSON.parse(await browser.evaluate(`JSON.stringify((() => {
          ...
          return { scale:getComputedStyle(document.querySelector('.app-shell')).zoom, transform:getComputedStyle(document.querySelector('.app-shell')).transform, oldScaleState:document.documentElement.dataset.homeScale, fonts:['.task-card-title','.task-meta','.task-edit','#selectedDateTitle','.brand h1'].map(s=>getComputedStyle(document.querySelector(s)).fontSize), canvas:rect('.app-shell'), width:innerWidth, overflow:document.documentElement.scrollWidth > innerWidth+1, cal:rect('.calendar-panel'), day:rect('.day-panel'), chat:rect('.chat-panel'), input:rect('#chatForm'), dayScroll:dayTasks.scrollHeight > dayTasks.clientHeight, spills:[...document.querySelectorAll('.calendar-panel,.day-panel,.chat-panel')].filter(n=>n.scrollWidth>n.clientWidth+1).length };
        })())`));
        assert.equal(geometry.overflow, false, JSON.stringify(geometry));
        assert.equal(geometry.spills, 0, JSON.stringify(geometry));
        assert.equal(geometry.scale, '1'); assert.equal(geometry.transform, 'none'); assert.equal(geometry.oldScaleState, undefined);
        assert.deepEqual(geometry.fonts, ['14px','12px','14px','18px','24px']);
        assert.ok(geometry.chat.w >= 290);
        assert.ok(Math.abs(geometry.canvas.w - Math.min(width, 1920)) < 1);
        assert.ok(Math.abs(geometry.canvas.x - (width - geometry.canvas.w) / 2) < 1);
