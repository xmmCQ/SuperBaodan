...
test("六条浏览器主链路使用临时数据和独立端口", { timeout: 20_000 }, async (t) => {
  ...
  await t.test("新建、切换、重命名和删除会话", async () => {
    ...
    await browser.evaluate(`document.querySelector('.session-row.active .session-more').click(); document.querySelector('.session-menu:popover-open [aria-label="重命名"]').click()`);
    ...
    await browser.evaluate(`document.querySelector('.session-row.active .session-more').click(); document.querySelector('.session-menu:popover-open .danger').click()`);
    await browser.waitFor("document.querySelector('#uiDialog').open");
    await browser.evaluate("uiDialogConfirm.click()");
